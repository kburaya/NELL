...
def define_words(text_corpus, result, parts_of_speech_1, parts_of_speech_2, logger, strict = False):
    ...
    try:
        category_dictionary[word_2] += 1
    except:
        category_dictionary.update({word_2: 1})
...
def main():
    ...
    parser.add_argument("-mode", type=str, help='[s/f] - single file or folder with texts')
    ...
    with open('results/сcategory_dictionary.json', 'w') as jsonfile:
        json.dump(category_dictionary, jsonfile, ensure_ascii=False)
